# Aurora — step‑by‑step setup guide
...
> **Remember this:** `source ~/aurora-venv/bin/activate` must be run **once in every new terminal window** before using `esphome` or the configurator. If you ever see `esphome: command not found`, this is why. (Your prompt shows `(aurora-venv)` when it's active.)
...
Open **<http://localhost:8765>** in your browser and sign in with the default password **`Admin`**.

> **Change the password right away** (⚙ Settings → Change password). The configurator is reachable by other devices on your network, and it stores a Home Assistant access token.

Then, inside the configurator:

1. **Connect to Home Assistant.** Enter your HA URL (e.g. `http://192.168.1.10:8123` — use the IP, not `.local`) and a **long‑lived access token**. To create one: in Home Assistant, click your user name (bottom‑left) → **Security** tab → scroll to **Long‑lived access tokens** → **Create token** → copy it.
2. Enter your **panel's IP address** (from Step 7).
3. Use the **entity wizard** to map each control (lights, locks, media, weather, presence…) to your own entities, and the **Rooms wizard** to set up your rooms.
4. Arrange cards on each page with the **drag‑and‑drop builder** — the preview matches the panel pixel‑for‑pixel.
5. Press **Flash** — the configurator builds your personalized firmware and sends it to the panel wirelessly.

Your design is saved on your computer (`layout.json`, `rooms.json`), so you can tweak and re‑flash any time.
...
Two habits keep it that way: **don't move those two files elsewhere**, and **change the configurator's default password** (Step 8). If you ever think your HA token leaked, delete it in Home Assistant (Security tab → your tokens) and create a new one.
...
| Panel boots but controls do nothing | The entities aren't yours yet — run the configurator's entity wizard (Step 8). |
| OTA update fails with "connection reset by peer" | Just retry — it's usually transient. |

More in the [README's Troubleshooting section](README.md#troubleshooting).
